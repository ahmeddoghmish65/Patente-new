                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-1 flex-wrap">
                      ...
                      {selectedUser.verified && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-primary-50 text-primary-600 flex items-center gap-0.5">
                          <Icon name="verified" size={12} filled /> موثق
                        </span>
                      )}
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-surface-100 p-5">
              ...
              <div className="flex items-end gap-2 h-40">
                {weekData.map((d, i) => {
                  const maxV = Math.max(...weekData.map(w => w.visits));
                  const height = maxV > 0 ? (d.visits / maxV) * 100 : 0;
                  const isToday = i === now.getDay();
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <span className="text-[10px] font-bold text-surface-600">{d.visits}</span>
                      <div className="w-full rounded-t-lg transition-all" style={{ height: `${height}%`, backgroundColor: isToday ? '#3b82f6' : '#e2e8f0' }} />
                      <span className={cn('text-[10px]', isToday ? 'font-bold text-primary-600' : 'text-surface-400')}>{d.day.substring(0, 3)}</span>
                    </div>
                  );
                })}
              </div>
